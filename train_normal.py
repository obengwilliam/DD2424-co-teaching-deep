...
def train(train_loader, epoch, model1, optimizer1, model2, optimizer2):
    ...
    for batch, (images, labels, indexes) in enumerate(train_loader):
        ...
        train_loss_1 = F.cross_entropy(pred_1, y, reduction='none')
...
def main():
    ...
    epoch = 0
    train_acc_1 = 0
    test_acc_1 = evaluate(test_loader, model1)
    train_acc_1 = evaluate(train_loader, model1)
    ...
    acc1 = evaluate(test_loader, model1)
    # save results
    ...
    for epoch in range(1, args.n_epoch):
        ...
        # train models
        train(train_loader, epoch, model1, optimizer1)
        # evaluate models
        test_acc_1 = evaluate(test_loader, model1)
        train_acc_1 = evaluate(train_loader, model1)
        wandb.log(
        ...
        print('Epoch [%d/%d] Test Accuracy on the %s test images: Model1 %.4f %% Model2 %.4f %%' %
              (epoch+1, args.n_epoch, len(test_dataset), test_acc_1))
